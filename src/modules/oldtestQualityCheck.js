...
function subsampleHeadings (positions) {
  ...
  for (let i = 0; i < positions.length; i++) {
    ...
    if (deltaT < 5000) {
      ...
      if (row.coords.heading !== undefined && !isNaN(row.coords.heading) && (row.coords.heading > 0) && row.timestamp !== undefined && !isNaN(row.timestamp)) {
        subsampledPositions.push(row)
      }
...
function classifyLogistic (positions) {
  ...
  console.log(`Subsampled ${subsampledPositions.length} positions`)
